 ...
 * @since 1.2
 */
public class IvoNetClassLoader extends SecureClassLoader implements Closeable {
     ...
     */
    public IvoNetClassLoader(final URL[] urls, final ClassLoader parent) {
        super(parent);
    ...
    IvoNetClassLoader(final URL[] urls, final ClassLoader parent,
     ...
     * @see SecurityManager#checkCreateClassLoader
     */
    public IvoNetClassLoader(final URL[] urls) {
        // this is to make the stack depth consistent with 1.1
        final SecurityManager security = System.getSecurityManager();
        if (security != null) {
            security.checkCreateClassLoader();
    ...
    IvoNetClassLoader(final URL[] urls, final AccessControlContext context) {
        // this is to make the stack depth consistent with 1.1
        final SecurityManager security = System.getSecurityManager();
        if (security != null) {
            security.checkCreateClassLoader();
     ...
     */
    public static IvoNetClassLoader newInstance(final URL[] urls,
                                                final ClassLoader parent) {
        // Save the caller's context
        final AccessControlContext acc = AccessController.getContext();
        // Need a privileged block to create the class loader
        return AccessController.doPrivileged(
                new PrivilegedAction<IvoNetClassLoader>() {
                    @Override
                    public IvoNetClassLoader run() {
     ...
     * <p/>
     * The method makes a best effort attempt to close all opened files, by catching {@link IOException}s internally.
     * Unchecked exceptions and errors are not caught. Calling close on an already closed loader has no effect.
     * <p/>
     *
     * @throws IOException       if closing any file opened by this class loader resulted in an IOException. Any such
